...
// For the sake of simplicity, some of the explanations here will ellide the use
// of matrix dimensions that aren't divisible by 4
  const shaderCode = `
    ...
    fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
        ...
        // When considering the functionality of the math at the beginning of this loop, it helps to consider variables and scalars separately. 
        // While variables (y, k) act as indentifiers for regions of the matrix we are accessing, the scalars use the identifiers to access the regions
        // the identifiers are more abstractly representing...
        // Identifiers:
        // y: Represents the index for a block of four rows within matrix A. It divides the matrix into blocks of 4 rows each. 
        //    Indices range from 0 to M rows / 4 (i.e the number of four row blocks in matrix A).
        // k: Represents the index for a block of four columns within matrix A. It divides the matrix into blocks of 4 columns each. 
        //    Indices range from 0 to K columns / 4 (i.e the number of four column blocks in matrix A)
        // Scalars and their usage:
        // y * 4u: Calculates the starting row for the current block 'y'. Each increment in 'y' moves the block down by 4 rows.
        // 0u, 1u, 2u, 3u: These are offsets used to access each of the four rows within the current block, ranging from 'y * 4' to 'y * 4 + 3'.
        // KD4: Defines the stride to align access within a row when accessing the matrix linearly (1-dimensional array representation).
        // k: Identifies the current 4-column block within the selected rows. 
        //    Accesses a vec4<f32> representing four consecutive elements in a row, corresponding to the columns of the block.
